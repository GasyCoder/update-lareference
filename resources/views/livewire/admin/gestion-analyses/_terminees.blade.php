{{-- resources/views/livewire/admin/gestion-analyses/_terminees.blade.php --}}
...
<div class="p-4">
    ...
    <div class="flex justify-between items-center mb-4">
        <div class="flex items-center space-x-3">
            <span
                class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-indigo-100 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200">
                <em class="ni ni-check-circle mr-2"></em>
                {{ $data->total() }} prescription(s) terminée(s)
            </span>
            <small class="text-gray-500 dark:text-gray-400 font-medium">En attente de validation par le
                biologiste</small>
        </div>
        ...
        <table class="w-full text-sm text-left text-slate-600 dark:text-slate-200">
            <thead
                ...
                <tr>
                    ...
                    <th class="px-6 py-4">Terminé le</th>
                    <th class="px-6 py-4 text-right">Actions</th>
                </tr>
            </thead>
            <tbody>
                @forelse($data as $prescription)
                    <tr wire:key="terminee-{{ $prescription->id }}"
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col text-xs">
                                <span
                                    class="text-slate-600 dark:text-slate-400">{{ $prescription->updated_at->format('d/m/Y') }}</span>
                    ...
                    <tr>
                        <td colspan="8" class="text-center py-12 text-slate-500 dark:text-slate-400">
                            <div class="flex flex-col items-center">
                                <em class="ni ni-clipboard-check text-5xl mb-4 text-indigo-500 dark:text-indigo-400"></em>
                                <p class="text-lg font-semibold text-slate-900 dark:text-slate-100">Aucune prescription
                                    terminée</p>
                                <p class="text-base text-slate-500 dark:text-slate-400">Aucune prescription n'est en attente
                                    de validation.</p>
                            </div>
                        </td>
                    </tr>
